...
var User = new Schema({
    name: String, email: String, gender: { type: String, enum: ['male', 'female'], default: 'male' }, age: { type: Number, default: 21 }, blogposts: [
        { type: ObjectId, ref: 'doc.populate.b' }
    ]
...
var Comment = new Schema({
    asers: [
        { type: ObjectId, ref: 'doc.populate.u' }
    ], _creator: { type: ObjectId, ref: 'doc.populate.u' }, content: String
});
...
var BlogPost = new Schema({
    _creator: { type: ObjectId, ref: 'doc.populate.u' }, title: String, comments: [Comment], fans: [
        { type: ObjectId, ref: 'doc.populate.u' }
    ]
});
...
describe('document.populate', function () {
    ...
    before(function (done) {
        ...
        User.create({
            ...
            B.create({
                title: 'the how and why', _creator: user1, fans: [user1, user2], comments: [
                    { _creator: user2, content: 'user2' },
                    { _creator: user1, content: 'user1' }
                ]
            }, function (err, p) {
                assert.ifError(err);
                post = p
    ...
    it('String _ids', function (done) {
        ...
        var NoteSchema = new Schema({
            author: { type: String, ref: 'UserWithStringId' }, body: String
        })
    ...
    it.skip('Buffer _ids', function (done) {
        ...
        var UserSchema = new Schema({
            _id: Buffer, name: String
        })

        var NoteSchema = new Schema({
            author: { type: Buffer, ref: 'UserWithBufferId' }, body: String
        })
    ...
    it('Number _ids', function (done) {
        ...
        var NoteSchema = new Schema({
            author: { type: Number, ref: 'UserWithNumberId' }, body: String
        })
