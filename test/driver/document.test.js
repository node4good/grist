...
var schema = new Schema({
    test: String, oids: [ObjectId], numbers: [Number], nested: {
        age: Number, cool: ObjectId, deep: { x: String }, path: String, setr: String
    }, nested2: {
        nested: String, yup: {
            nested: Boolean, yup: String, age: Number
        }
    }, em: [em], date: Date
});
...
describe('document:', function () {
    ...
    describe('shortcut getters', function () {
        ...
        it.skip('work', function (done) {
            var doc = new TestDocument();
            doc.init({
                test: 'test', oids: [], nested: {
                    age: 5, cool: DocumentObjectId.createFromHexString('4c6c2d6240ced95d0e00003c'), path: 'my path'
                }
            });
            ...
            doc2.init({
                test: 'toop', oids: [], nested: {
                    age: 2, cool: DocumentObjectId.createFromHexString('4cf70857337498f95900001c'), deep: { x: 'yay' }
                }
            });
            ...
            doc2.nested2.yup = {
                age: 150, yup: "Yesiree", nested: true
            };
    ...
    it('test toObject clone', function (done) {
        ...
        doc.init({
            test: 'test', oids: [], nested: {
                age: 5, cool: new DocumentObjectId
            }
        });
    ...
    it.skip('toObject options', function (done) {
        ...
        clone = doc.toObject({
            transform: xform, fields: '_id em numbers oids nested'
        });
        assert.equal('test', doc.test);
        assert.ok(undefined === clone.em);
        assert.ok(undefined === clone.numbers);
    ...
    it('no maxListeners warning should occur', function (done) {
        ...
        var schema = new Schema({
            title: String, embed1: [new Schema({name: String})], embed2: [new Schema({name: String})], embed3: [new Schema({name: String})], embed4: [new Schema({name: String})], embed5: [new Schema({name: String})], embed6: [new Schema({name: String})], embed7: [new Schema({name: String})], embed8: [new Schema({name: String})], embed9: [new Schema({name: String})], embed10: [new Schema({name: String})], embed11: [new Schema({name: String})]
        });
    ...
    it('isSelected()', function (done) {
        ...
        doc.init({
            test: 'test', numbers: [4, 5, 6, 7], nested: {
                age: 5, cool: DocumentObjectId.createFromHexString('4c6c2d6240ced95d0e00003c'), path: 'my path', deep: { x: 'a string' }
            }, notapath: 'i am not in the schema', em: [
                { title: 'gocars' }
            ]
        });
        ...
        var selection = {
            'test': 1, 'numbers': 1, 'nested.deep': 1, 'oids': 1
        }
    ...
    describe('#validate', function () {
        ...
        it('works (gh-891)', function (done) {
            ...
            schema = new Schema({
                prop: { type: String, required: true, validate: validate }, nick: { type: String, required: true }
            });
        ...
        describe('works on arrays', function () {
            ...
            it('with required', function (done) {
                var schema = new Schema({
                    name: String, arr: { type: [], required: true}
                });
