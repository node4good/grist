...
describe('schema', function () {
    ...
    it('supports different schematypes', function (done) {
        ...
        var Ferret = new Schema({
            name: String, owner: ObjectId, fur: String, color: { type: String }, age: Number, checkins: [Checkin], friends: [ObjectId], likes: Array, alive: Boolean, extra: Mixed
        });
        ...
        var Ferret1 = new Schema({
            name: "string", owner: "oid", fur: { type: "string" }, color: { type: "String" }, checkins: [Checkin], friends: Array, likes: "array", alive: "Bool", alive1: "bool", alive2: "boolean", extra: "mixed", obj: "object", buf: "buffer", Buf: "Buffer"
        });
    ...
    it('supports dot notation for path accessors', function (done) {
        var Racoon = new Schema({
            name: { type: String, enum: ['Edwald', 'Tobi'] }, age: Number
        });

        var Person = new Schema({
            name: String, raccoons: [Racoon], location: {
    ...
    it('default definition', function (done) {
        var Test = new Schema({
            simple: { type: String, default: 'a' }, array: { type: Array, default: [1, 2, 3, 4, 5] }, arrayX: { type: Array, default: 9 }, arrayFn: { type: Array, default: function () {
                return [8]
    ...
    it('Mixed defaults can be empty arrays', function (done) {
        var Test = new Schema({
            mixed1: { type: Mixed, default: [] }, mixed2: { type: Mixed, default: Array }
        });
    ...
    describe('casting', function () {
        ...
        it('array', function (done) {
            var Loki = new Schema({
                oids: [ObjectId], dates: [Date], numbers: [Number], strings: [String], buffers: [Buffer], nocast: [], mixed: [Mixed]
            });
    ...
    describe('indexes', function () {
        describe('definition', function () {
            ...
            it('compound', function (done) {
                var Tobi = new Schema({
                    name: { type: String, index: true }, last: { type: Number, sparse: true }, nope: { type: String, index: { background: false }}
                });
                ...
                assert.deepEqual(Tobi.indexes(), [
                    ...
                    [
                        { firstname: 1, last: 1},
                        {unique: true, expireAfterSeconds: 60 * 60, background: true }
                    ]
                    ,
                    [
                        { firstname: 1, nope: 1 },
    ...
    describe('construction', function () {
        ...
        it('does not alter original argument (gh-1364)', function (done) {
            var schema = {
                ids: [
                    { type: Schema.ObjectId, ref: 'something' }
                ], a: { type: Array }, b: Array, c: [Date], d: { type: 'Boolean' }, e: [
                    { a: String, b: [
                        { type: { type: Buffer }, x: Number }
                    ] }
                ]
            };
    ...
    describe('pathType()', function () {
        ...
        before(function () {
            schema = Schema({
                n: String, nest: { thing: { nests: Boolean }}, docs: [
                    { x: [
                        { y: String }
                    ] }
                ], mixed: {}
