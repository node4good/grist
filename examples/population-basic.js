...
var consoleSchema = Schema({
  name: String, manufacturer: String, released: Date
})
...
var gameSchema = Schema({
  name: String, developer: String, released: Date, consoles: [
...
function createData() {
  Console.create({
    ...
    Game.create({
      name: 'Legend of Zelda: Ocarina of Time', developer: 'Nintendo', released: new Date('November 21, 1998'), consoles: [nintendo64]
    }, function (err) {
      if (err) return done(err);
      example();
    })
