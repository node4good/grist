...
Collection.prototype.persist = function (message) {
    var self = this;
    var basename = path.basename(self._filename, '.json');
    var gitDir = path.join(this._tdb._path, basename, '.git');
    if (!fs.existsSync(this._tdb._path)) fs.mkdirSync(this._tdb._path);
    if (!fs.existsSync(path.join(this._tdb._path, basename))) fs.mkdirSync(path.join(this._tdb._path, basename));
    ...
    var p = Promise.fulfilled().then(
        function () {
            ...
            repo.setHead("master", function (err) {
                ...
                self.repo.saveAs("tree", tree, function (err, treeHash) {
                    if (err) throw err;
                    var commit = _.assign({ tree: treeHash, message: "initial" }, COMMIT_TEMPLATE);
                    repo.saveAs("commit", commit, function (err, commitHash) {
                        pi.fulfill(commitHash);
                    });
                });
            });
            return pi;
        ...
        function (headHash) {
            ...
            self.repo.saveAs("blob", data, function (err, blobHash) {
                ...
                self.repo.saveAs("tree", tree, function (err, treeHash) {
                    if (err) throw err;
                    self.repo.loadAs('commit', headHash, function (err, headCommit) {
                        if (err) throw err;
                        var commit = _.assign({ tree: treeHash, parent: headCommit.tree, message: message }, COMMIT_TEMPLATE);
...
Collection.prototype._remove = function (item) {
    ...
    var p = Promise
        .fulfilled()
        .then(
            this.persist.bind(this, 'Removed ' + item._id)
        ).then(
        function () {
            return 1;
        }
...
Collection.prototype._remove_batch = function (items) {
    ...
    var p = Promise
        .fulfilled()
        .then(
            this.persist.bind(this, 'Removed ' + items.length)
        ).then(
        function () {
            return items.length;
        });
...
function handleKeys(query, store) {
    ...
    delete query._id;
    if (!id_query) return _.values(store);

    if (!_.isPlainObject(id_query)) {
        var key = JSON.stringify(id_query).replace(/"/g, '');
        var val = store[key];
        return _.compact([val]);
    }

    _.walk.preorder(id_query, function (value, key, parent) {
        if (value instanceof ObjectId) parent[key] = value.toString();
    });
    var keys = sift(id_query, _.keys(store));
