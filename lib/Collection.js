...
function Collection(tdb, name) {
    if (!tdb || !name) throw new TypeError("Need the db, and the collection name.");
    this._store = {};
    this._id = 1;
    this._tq = null;
    this._idx = {};
...
Collection.prototype.init = function (cb) {
    this._init_store();
    return Promise.fulfilled(this).onResolve(cb);
};
