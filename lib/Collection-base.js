...
Collection.prototype.init = function (tdb, name, options, callback) {
    ...
    this._tq = new wqueue(100, function (qcb) {
        self._init_store(0, 0, function () {
            // update indexes
            async.forEachSeries(
                _.values(self._store),
                function (rec, acb) {
                    self._get(rec.pos, function (err, obj) {
    ...
    self.ensureIndex({_id: 1}, {unique: true}, callback);
};
...
Collection.prototype._put = function (item, remove, cb) {
    ...
    this._put_write(self, sobj, key, item, buf, remove, function (err) {
        if (err) throw err;
        // update index
