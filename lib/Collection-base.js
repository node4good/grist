...
Collection.prototype.drop = function (cb) {
    this._store = {};
    this._remove_batch([]).onResolve(cb);
};
...
Collection.prototype.update = function (query, doc, opts, callback) {
    ...
    return Promise.fulfilled().then(function () {
        ...
        var updater = new Updater(doc, self._tdb, multi);
        var $doc = updater.hasAtomic() ? null : doc;
        var res = self._find(query, null, 0, multi ? null : 1);
        if (_.isEmpty(res)) {
            if (!opts.upsert) return Promise.fulfilled(0);
            $doc = $doc || query;
        ...
        var udocs = res.map(function (obj) {
            var udoc = $doc;
            if (!$doc) {
                udoc = obj;
                updater.update(udoc);
        ...
        var pr = self._put_batch(udocs).then(
            function (uudocs) {
                return Promise.fulfilled(uudocs.length, {updatedExisting: true, n: uudocs.length});
            }
        );
        return pr.end();
...
Collection.prototype.findAndModify = function (query, sort, doc, opts, callback) {
    ...
    return Promise.fulfilled().then(function () {
        if (cursor._err) throw cursor._err;
        return self._find(query, null, 0, 1, cursor._sort, cursor._order);
    }).then(function (res) {
        if (!opts.remove) {
            return res;
        } else return self._remove_batch(res).then(function (cnt) {
            if (cnt != 1) throw new Error("not removed");
            return res;
        });
    }).then(function (res) {
        if (!_.isEmpty(res)) return self._tdb._cloneDeep(res[0]);
        if (!opts.upsert) return null;

        var $doc = updater.hasAtomic() ? query : self._tdb._cloneDeep(doc);
        updater.update($doc, true);
        if (_.isUndefined($doc._id)) $doc._id = new self._tdb.ObjectID();
        return $doc;
    }).then(function (obj) {
        var udoc = updater.update(obj);
        udoc._id = obj._id;
        return self._put(udoc);
    }).then(function (doc) {
        if (!doc) return null;
        return cursor._projectFields(doc);
    }).onResolve(callback).end();
};
