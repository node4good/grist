...
Collection.prototype.update = function (query, doc, opts, callback) {
    ...
    var p = Promise.fulfilled().then(function () {
        ...
        if (!_.isObject(doc)) throw new Error("document must be a valid JavaScript object");
        var udocs;
        ...
        if (_.isEmpty(res)) {
            ...
            meta = {updatedExisting: false, upserted: $doc._id};
            udocs = [$doc];
        } else {
...
Collection.prototype.findAndRemove = function (query, sort, opts, cb) {
    ...
    return this._remove_batch(res).then(function () { return ret; }).onResolve(cb);
};
