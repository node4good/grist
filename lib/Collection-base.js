...
Collection.prototype.update = function (query, doc, opts, callback) {
    ...
    return Promise.fulfilled().then(function () {
        ...
        var pr = self._put_batch(udocs).then(
            function (uudocs) {
                return new Promise().fulfill(uudocs.length, {updatedExisting: true, n: uudocs.length}).end();
            }
        );
        return pr.end();
...
Collection.prototype.findAndModify = function (query, sort, doc, opts, callback) {
    ...
    c.sort(sort);
    if (c._err)
        return safe.back(callback, c._err);

    var res = self._find(query, null, 0, 1, c._sort, c._order);
    if (opts.remove) {
        return self._remove_batch(res).onResolve(function (err, cnt) {
            if (cnt != 1) err = new Error("not removed");
            callback(err, res[0]);
        });
    }
    if (_.isEmpty(res)) {
        if (!opts.upsert) return callback();
        $doc = $doc || query;
        $doc = self._tdb._cloneDeep($doc);
        updater.update($doc, true);
        if (_.isUndefined($doc._id))
            $doc._id = new self._tdb.ObjectID();
        self._put($doc).onFulfill(function () {
            callback(null, opts.new ? c._projectFields($doc) : {});
        }).end();
        return;
    }
    ...
    udoc._id = obj._id;
    robj = c._projectFields(robj);
    self._put(udoc).onFulfill(function () {
        callback(null, robj);
    }).end();
};
