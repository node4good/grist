...
Collection.prototype.update = function (query, doc, opts, cb) {
    ...
    self._find(query, null, 0, multi ? null : 1, null, null, safe.sure(cb, function (res) {
        if (_.isEmpty(res)) {
...
Collection.prototype.remove = function (query, opts, callback) {
    ...
    var p = self._find(query, null, 0, single ? 1 : null, null, null
        ).then(
        function (res) {
            res.forEach(
                function (obj) {
                    p = p.then(
                        function () {return self._get(obj);}
                    ).then(
                        function (obj) { return self._put(obj, true); }
                    );
                }
            );
        }
