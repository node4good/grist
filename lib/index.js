...
TDB.prototype.dropDatabase = function (callback) {
    function fsWalkStrategy(name) {
        return fs.statSync(name).isDirectory() ? fs.readdirSync(name).map(function (n) {
            return Path.join(name, n);
        }) : [];
    }

    _.walk(fsWalkStrategy).reduce(
        this._path,
        function (ifLeaf, node) {
            if (ifLeaf && fs.statSync(node).isDirectory()) fs.rmdirSync(node);
            else fs.unlinkSync(node);
            return true;
        },
        true
    );
    return Promise.fulfilled().onResolve(callback);
};
