...
Cursor.prototype.count = function (applySkipLimit, cb) {
    ...
    self._c._find(self._query, {}, 0, null, null, null, safe.sure(cb, function (data) {
...
Cursor.prototype.toArray = function (cb) {
    ...
    self._ensure(safe.sure(cb, function () {
        ...
        async.mapSeries(self._items.slice(iteratorValue), function (pos, cb) {
            self._get(pos, function (err, obj) {
                if (err) throw err;
                cb(null, obj);
            });
        }, function (err, res) {
...
Cursor.prototype._ensure = function (cb) {
    ...
    self._c._find(self._query, {}, self._skip, self._limit, self._sort, self._order, function (err, data) {
        if (err) throw err;
        self._items = data;
        self._i = 0;
        cb(null, data);
